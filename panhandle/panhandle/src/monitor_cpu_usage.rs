...
// cpu monitoring helper function - single sample, state passed externally
pub async fn monitor_cpu_usage(
    ...
    last_pid_times: &mut StdHashMap<u32, u64>,
    pid_stats: &mut StdHashMap<u32, PidStats>,
    ...
    *sample_count += 1;
    ...
    // Use saturating_sub to prevent underflow from clock anomalies
    ...
    if let Some(pids_to_check) = pid_filter {
        ...
        // Global CPU monitoring - system-wide stats only (not all PIDs)
        let total_cpu_time_available = if poll_interval > 0 && num_cpus > 0 {
            let interval_ns = (poll_interval as u64).saturating_mul(1_000_000_000);
...
/// Print CPU monitoring summary statistics
pub async fn print_cpu_summary(
    _pid_filter: &Option<Vec<u32>>,
    num_cpus: usize,
    sample_count: u64,
    poll_interval: u32,
    hostname: &Arc<String>,
    ...
    json_output: &bool,
    debug: &bool,
    pid_stats: &StdHashMap<u32, PidStats>,
    global_stats: &mut GlobalStats,
    _last_total_busy: u64,
) {
    let timestamp = Utc::now().to_rfc3339();

    if !pid_stats.is_empty() {
        // Per-PID summary
        let duration_sec = sample_count.saturating_mul(poll_interval as u64);
        let plain_string = format!(
