...
use crate::*;
/// this is the ebpf program to access the fmsh entries via
/// the readline or readline teardown method.

#[map(name = "fmsh_events")]
static mut FMSH_EVENTS: PerfEventArray<Readline> = PerfEventArray::new(0);
#[map(name = "fmsh_scratch")]
pub static FMSH_SCRATCH: PerCpuArray<Readline> = PerCpuArray::with_max_entries(4096, 0);
#[map(name = "fmsh_uid_options")]
static FMSH_UID_OPTIONS: HashMap<u32, u32> = HashMap::<u32, u32>::with_max_entries(4, 0);
#[map(name = "fmsh_uid_include_list")]
static FMSH_UID_INCLUDE_LIST: HashMap<u32, [u32; UID_COUNT]> =
    HashMap::<u32, [u32; UID_COUNT]>::with_max_entries(1, 0);

#[uretprobe]
pub fn fmsh(ctx: RetProbeContext) -> u32 {
    match try_fmsh(ctx) {
        Ok(ret) => ret,
        Err(ret) => ret as u32,
    }
...
fn try_fmsh(ctx: RetProbeContext) -> Result<u32, i64> {
    ...
    if exclude_uid(initial_uid, &FMSH_UID_OPTIONS) {
        return Ok(0);
    }
    ...
    if get_bool(3, &FMSH_UID_OPTIONS) {
        if !check_uid_in_uidarray(&initial_uid, &FMSH_UID_INCLUDE_LIST) {
            return Ok(0);
        }
    }
    ...
    let event: &mut Readline = unsafe {
        let ptr: *mut Readline = FMSH_SCRATCH.get_ptr_mut(0).ok_or(0)?;
        &mut *ptr
    };
    ...
    *event = unsafe { core::mem::zeroed::<Readline>() };
    ...
    // the map is created on program load
    unsafe {
        FMSH_EVENTS.output(&ctx, event, 0);
    }
