...
fn try_readline(ctx: RetProbeContext) -> Result<u32, i64> {
    ...
    // - For readline, this is a pointer to the input string
    // - In eBPF uretprobe context, this should be a valid pointer or null
    let ret_ptr: *const u8 = ctx.ret();
    // Null pointer check: ret_ptr must not be null for valid readline return
    if ret_ptr.is_null() {
        return Ok(0);
    }
    ...
    // - READLINE_SCRATCH is a PerCpuArray<Readline> with max_entries=4096, created at program load
    ...
    *event = unsafe { core::mem::zeroed::<Readline>() };
    ...
    // - ret_ptr points to valid user space memory (return value from readline function)
    ...
    // - command() returns the path of the probed function (bash/zsh readline)
    ...
    // - READLINE_EVENTS is a properly initialized PerfEventArray<Readline>
